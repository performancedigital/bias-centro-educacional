...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-bias-yellow selection:text-bias-blue overflow-x-hidden font-sans text-slate-900 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 group cursor-pointer">
              <img alt="BIAS Logo" className="w-12 h-12 object-contain transition-all duration-500 group-hover:scale-110 group-hover:rotate-3" src={LOGO_URL} />
              <div className="leading-none">
                <span className="text-2xl font-black text-bias-blue tracking-tighter uppercase">BIAS</span>
                <p className="text-[8px] uppercase tracking-[0.2em] text-bias-blue font-bold opacity-70">Centro Educacional</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 md:gap-10">
            ...
            <button onClick={() => handleWhatsApp()} className="bg-bias-blue text-white px-8 py-3.5 rounded-2xl font-black text-xs md:text-sm hover:bg-bias-navy hover:shadow-[0_10px_30px_rgba(0,71,171,0.3)] transition-all uppercase tracking-widest active:scale-95">
              FALAR COM CONSULTOR
            </button>
          </div>
        </div>
      ...
      {/* Hero Section */}
      <header className="relative pt-40 pb-32 md:pt-56 md:pb-48 px-4 z-10">
        <div className="max-w-5xl mx-auto text-center reveal active">
          <div className="inline-flex items-center bg-bias-blue/5 text-bias-blue border border-bias-blue/10 px-6 py-2.5 rounded-full text-[11px] font-black mb-12 uppercase tracking-[0.2em] shadow-sm animate-pulse-soft">
            <span className="flex h-2.5 w-2.5 rounded-full bg-green-500 mr-3 shadow-[0_0_10px_#22c55e]"></span>
            🚀 MATRÍCULAS ABERTAS 2026
          </div>
          
          <h1 className="text-5xl md:text-7xl font-black text-gradient leading-[1.05] mb-14 tracking-tighter uppercase">
            Sua formação técnica<br />em apenas 6 meses.
          </h1>

          <div className="max-w-2xl mx-auto mb-16 p-1.5 bg-gradient-to-r from-bias-blue/10 via-bias-yellow/10 to-bias-blue/10 rounded-[2.5rem] shadow-inner transition-all hover:shadow-lg">
            <div className="px-10 py-8 bg-white/90 backdrop-blur rounded-[2.3rem] shadow-sm border border-white">
              <p className="text-xl md:text-2xl font-bold text-slate-600 uppercase tracking-tight">
                Invista no seu futuro por apenas<br className="md:hidden" />
                <span className="text-bias-blue font-black text-3xl md:text-4xl ml-2 block md:inline-block mt-4 md:mt-0 drop-shadow-sm">{PRICE_INSTALLMENT}</span>
              </p>
            </div>
          </div>

          <button onClick={() => handleWhatsApp()} className="bg-bias-blue text-white px-16 py-7 rounded-[2rem] font-black text-xl md:text-2xl shadow-[0_20px_50px_rgba(0,71,171,0.3)] hover:bg-bias-navy hover:scale-105 active:scale-95 transition-all uppercase tracking-tight relative overflow-hidden group hover-shine">
            <span className="relative z-10">QUERO GARANTIR MINHA VAGA</span>
          </button>
        </div>

        {/* Floating Trust Pill */}
        <div className="mt-24 flex flex-wrap justify-center gap-8 md:gap-16 opacity-50 grayscale hover:grayscale-0 transition-all duration-700 reveal">
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase tracking-widest text-bias-blue">RECONHECIDO</span>
             <span className="text-2xl font-black text-bias-navy">MEC</span>
          </div>
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase tracking-widest text-bias-yellow">REGISTRADO</span>
             <span className="text-2xl font-black text-bias-navy">SISTEC</span>
          </div>
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase tracking-widest text-bias-blue">SUPORTE</span>
             <span className="text-2xl font-black text-bias-navy">ALUNO</span>
          </div>
        </div>
      </header>
      ...
      <div className="w-full overflow-hidden leading-[0] fill-slate-50 relative z-10 -mt-2">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="w-full h-16 md:h-24 transform rotate-180">
      ...
      <section id="cursos" className="py-32 bg-slate-50 relative z-10">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-24 reveal">
            <h2 className="text-5xl md:text-6xl font-black text-bias-blue mb-6 uppercase tracking-tighter">FORMAÇÕES EM DESTAQUE</h2>
            <div className="w-24 h-2 bg-bias-yellow mx-auto rounded-full mb-8"></div>
      ...
      {/* Lead Filter Section */}
      <div className="reveal relative z-10 py-16">
        <LeadFilter />
      </div>

      {/* Diploma por Experiência Section */}
      <section id="competencia" className="py-40 relative text-white overflow-hidden reveal group/industrial">
            ...
            {TESTIMONIALS.map(t => (
              <div key={t.id} className="reveal bg-slate-50/70 p-14 rounded-[4rem] border border-slate-100 hover:bg-white hover:shadow-[0_40px_100px_rgba(0,71,171,0.08)] transition-all duration-700 group">
                <div className="flex gap-1 mb-8">
                  {[1,2,3,4,5].map(star => <span key={star} className="text-bias-yellow text-xl">★</span>)}
                </div>
                <p className="text-xl font-medium italic text-slate-600 mb-12 leading-relaxed">"{t.content}"</p>
                <div className="flex items-center gap-6">
                  <div className="relative">
                    <img alt={t.name} className="w-16 h-16 rounded-[1.5rem] border-2 border-bias-yellow grayscale group-hover:grayscale-0 transition-all duration-700 object-cover" src={t.avatar} />
                    <div className="absolute -bottom-2 -right-2 bg-green-500 w-5 h-5 rounded-full border-4 border-white shadow-sm"></div>
                  </div>
                  <div>
                    <p className="font-black text-bias-blue text-lg uppercase tracking-tight leading-none mb-2">{t.name}</p>
      ...
      {/* Authority Ticker */}
      <div className="w-full bg-bias-blue py-10 overflow-hidden z-10 relative">
        <div className="flex animate-marquee whitespace-nowrap">
          {[1,2,3,4].map(i => (
            <div key={i} className="flex items-center gap-32 mx-16">
              <span className="text-white/60 font-black text-2xl uppercase tracking-tighter flex items-center gap-4">
                <span className="text-bias-yellow text-4xl">✓</span> DIPLOMA RECONHECIDO MEC
              </span>
              <span className="text-white/60 font-black text-2xl uppercase tracking-tighter flex items-center gap-4">
                <span className="text-bias-yellow text-4xl">✓</span> REGISTRO NO CONSELHO (CREA/CFT)
              </span>
              <span className="text-white/60 font-black text-2xl uppercase tracking-tighter flex items-center gap-4">
                <span className="text-bias-yellow text-4xl">✓</span> SUPORTE VIP VIA WHATSAPP
              </span>
            </div>
          ))}
        </div>
      ...
      {/* Footer */}
      <footer className="bg-slate-950 text-white py-32 relative z-10">
        <div className="max-w-7xl mx-auto px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-20 text-center md:text-left">
            ...
            <div className="reveal">
              <h4 className="font-black text-xs mb-10 uppercase tracking-[0.4em] text-bias-yellow">LINKS RÁPIDOS</h4>
            ...
            <div className="reveal">
              <h4 className="font-black text-xs mb-10 uppercase tracking-[0.4em] text-bias-yellow">UNIDADE FÍSICA</h4>
              <p className="text-slate-400 text-[12px] font-bold leading-relaxed uppercase tracking-widest">
      ...
      <div className={`fixed bottom-8 left-8 z-[200] transition-all duration-700 ${notification.visible ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}>
        <div className="bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-2xl border border-slate-100 flex items-center gap-4 max-w-sm">
          <div className="w-10 h-10 bg-bias-blue/10 rounded-full flex items-center justify-center text-xl">🎓</div>
          <p className="text-xs font-black text-bias-blue uppercase tracking-tight">{notification.text}</p>
        </div>
      </div>

      {/* Floating WhatsApp Button */}
      <button onClick={() => handleWhatsApp()} className="fixed bottom-8 right-8 z-[200] bg-[#25D366] text-white w-18 h-18 md:w-20 md:h-20 rounded-full flex items-center justify-center shadow-[0_15px_40px_rgba(37,211,102,0.4)] hover:scale-110 active:scale-95 transition-all animate-float">
